...
%     Khh=[];
    for i=1:NumFilesChunk
        ...
        if strcmpi(conf.Traj.Resuspension,'yes') %ZC 03/02/20
           TAUC=cat(3,TAUC,dataOGCM_aux.TAUC);
           Kvv=cat(4,Kvv,dataOGCM_aux.Kv);
%            Khh=cat(4,Kvv,dataOGCM_aux.Kh);
        end
    ...
    % Initialize Resuspension variables ZC
    if strcmpi(conf.Traj.Resuspension,'yes')
            Bottomstress_c=conf.Traj.Bs_ini;
               TAUC1=[];
               for jj=1:conf.Traj.tauclon
                 for ii=1:conf.Traj.tauclat
                  TAUC1(ii,jj,:) = interp1(TT,squeeze(TAUC(ii,jj,:)),tspan_chunk,'spline'); %IJR change nearest by spline
                 end
               end
   %------ZC Kvv interpolate
               Kvv1=[];
               for jj=1:conf.Traj.tauclon
                 for ii=1:conf.Traj.tauclat
                     for kk=1:conf.Traj.kb+2
                      Kvv1(ii,jj,kk,:) = interp1(TT,squeeze(Kvv(ii,jj,kk,:)),tspan_chunk,'spline');
                     end
                  end      
               end 
    end
    resuspension=zeros(numpar,length(tspan_chunk)); %resuspension matrix [particles, time] equal to 1 when a particles is resuspended at a given time
    ...
    for i=IniLoop:length(tspan_chunk)
        ...
        if strcmpi(conf.Traj.Resuspension,'yes') % ZC 03/02/20
            ...
            for re=1:numpar           
                   if FateType(re)==2 
                      if chunk~=1 && cont_timechunk==1 
                         TAUCP=TAUC1(:,:,1);
                         Tauc_partLonLat=griddata(Lon(mask_water==1),Lat(mask_water==1),TAUCP(mask_water==1),LL(re,1),LL(re,2),'linear');
                      else   
                         TAUCP=TAUC1(:,:,i);
                         Tauc_partLonLat=griddata(Lon(mask_water==1),Lat(mask_water==1),TAUCP(mask_water==1),PosResus(re,1),PosResus(re,2),'linear');
                      end
                      
                      if Tauc_partLonLat>=Bottomstress_c 
                       resuspension(re,i)=1; 
...
%         end
       ...
       %ZC Kv variable   03/02/20
      
        for part=1:numpar
            if ~isnan(sum(LL(part,:)))&& strcmpi(conf.Traj.Resuspension,'yes') %for resuspension only for now
                Kvv2=Kvv1(:,:,:,i); 
                for pp=1:conf.Traj.kb+2
                 Kv_n(pp)=interp2(Lon,Lat,Kvv2(:,:,pp),LL(part,1),LL(part,2));
                 depthKv(pp)=interp2(Lon,Lat,Depth(:,:,pp),LL(part,1),LL(part,2));  
                end
                Kv=interp1(depthKv,Kv_n,LL(part,3));
            else  
            Kv=conf.Traj.Kv;        
            end
        end
        
        Kv(Kv<=0)=conf.Traj.Kv;  %ZC 18/08/19
        Kv(isnan(Kv))=conf.Traj.Kv;  %for start of every chunk, to be discussed
        
        [TurbHx, TurbHy, TurbV] = HTurb30(numpar,TimeStepOut*24*60*60,'Kh',conf.Traj.Kh,'Kv',Kv); %ZC 03/02/20
        ...
        for part=1:numpar
            ...
            % Settling check    ZC 03/02/20 
            
            % Here -Depth_partLonLat*0.15 is neccesary to remove the difference from different interpolation in ode and depth
            % when dep=10m the difference sometimes can be 1.03 m so here use 15%(0.15)
            
            elseif abs(h_TOT(i,part))>=Depth_partLonLat && resuspension(part,i)==0 && strcmpi(conf.Traj.Resuspension,'yes') %
                ln_TOT(i,part)=NaN; lt_TOT(i,part)=NaN;  h_TOT(i,part)=NaN;
                FateType(part)=2;
                TimeLand(part)=ts_TOT(i);            
               if chunk~=1 && cont_timechunk==1 %the last postion of the first time step of a chunk is equal to LL (IJR 26/06/2019)
                    PosResus(part,1)=LL(part,1); %Save the last postion of particles in the water, for resuspension
                    PosResus(part,2)=LL(part,2); %Save the last postion of particles in the water, for resuspension
                    PosResus(part,3)=LL(part,3);
               else
                    PosResus(part,1)=ln_TOT(i-1,part); %Save the last postion of particles in the water, for resuspension ZC
                    PosResus(part,2)=lt_TOT(i-1,part); %Save the last postion of particles in the water, for resuspension ZC
                   %PosResus(part,3)=h_TOT(i-1,part); %-DepthBottomTraj(part,i-1)+0.05; %Depth should be in the same time step with PosResus
                    PosResus(part,3)=-DepthBottomTraj(part,i-1)+Depth_partLonLat*0.15; %Depth should be in the same time step with PosResus 
               end  
               
             elseif abs(h_TOT(i,part))>=Depth_partLonLat && strcmpi(conf.Traj.Resuspension,'no')
                ln_TOT(i,part)=NaN; lt_TOT(i,part)=NaN;  h_TOT(i,part)=NaN;
                FateType(part)=2;
            ...
            % Resuspended particle ZC 03/02/20
            elseif resuspension(part,i)==1 
                ln_TOT(i,part)=PosResus(part,1); %IJR (OPTIONAL) comment if want to eat step 2 
                lt_TOT(i,part)=PosResus(part,2); %IJR (OPTIONAL) comment if want to eat step 2 
                h_TOT(i,part)=PosResus(part,3);  %IJR (OPTIONAL) comment if want to eat step 2 
                FateType(part)=0; %Water for resuspension part ZC   
            end   
            % Avoid particles out of water because or turbulence/sinking
            if h_TOT(i,part)>=0
            ...
            DepthBottomTraj(part,i)=griddata( Lon(mask_water==1),Lat(mask_water==1),H(mask_water==1),ln_TOT(i,part),lt_TOT(i,part),'linear'); %ZC 03/02/20
